...
if [ \"\$FLAVOR\" != \"local\" ]; then
    /opt/mcAI/bin/python $HOME/main.py $HOME/configure.json
else
...
installPackages() {
    ...
    DEBIAN_FRONTEND=noninteractive sudo apt install python3 python3-pip python-is-python3 python3-venv watchdog -y > /dev/null
