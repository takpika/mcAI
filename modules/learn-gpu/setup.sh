...
if [ \"\$FLAVOR\" != \"local\" ]; then
    ...
    /opt/mcAI/bin/python -m debugpy --wait-for-client --listen 0.0.0.0:12888 mcAI/modules/$MODULE/main.py $HOME/configure.json
fi
"
...
installPackages() {
    ...
    DEBIAN_FRONTEND=noninteractive sudo apt install python3 python-is-python3.11 python3-pip python3.11-venv python3-dev git cifs-utils inetutils-ping watchdog libgl1-mesa-dev libglib2.0-0 -y > /dev/null
